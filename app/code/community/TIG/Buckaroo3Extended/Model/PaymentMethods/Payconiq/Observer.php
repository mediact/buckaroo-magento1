...
class TIG_Buckaroo3Extended_Model_PaymentMethods_Payconiq_Observer extends TIG_Buckaroo3Extended_Model_Observer_Abstract
{
    ...
    public function buckaroo3extended_push_custom_processing_after(Varien_Event_Observer $observer)
    {
        ...
        if(!isset($push['brq_SERVICE_payconiq_ProtectionEligibility']) &&
            !isset($push['brq_service_payconiq_ProtectionEligibility'])) {
                return $this;
        }

        if(!isset($push['brq_SERVICE_payconiq_ProtectionEligibility'])){
            $eligibility = $push['brq_service_payconiq_ProtectionEligibility'];
        } else {
            $eligibility = $push['brq_SERVICE_payconiq_ProtectionEligibility'];
        }

        if(!isset($push['brq_SERVICE_payconiq_ProtectionEligibilityType'])){
            $eligibilityType = $push['brq_service_payconiq_ProtectionEligibilityType'];
        } else {
            $eligibilityType = $push['brq_SERVICE_payconiq_ProtectionEligibilityType'];
        }
        ...
        $commentEligible = Mage::helper('buckaroo3extended')->__(
            'Merchant is protected by Payconiqs Seller Protection Policy for both Unauthorized Payment and Item Not Received'
        );
        $commentItemNotReceivedEligible = Mage::helper('buckaroo3extended')->__(
            'Merchant is protected by Payconiqs Seller Protection Policy for Item Not Received'
        );
        $commentUnauthorizedPaymentEligible = Mage::helper('buckaroo3extended')->__(
            'Merchant is protected by Payconiqs Seller Protection Policy for Unauthorized Payment'
        );
        ...
        switch ($eligibilityType) {
            ...
            case 'Eligible':
                $eligibilityStatus = Mage::getStoreConfig(
                    'buckaroo/buckaroo3extended_payconiq/sellers_protection_eligible',
                    $order->getStoreId()
                );
            ...
            case 'ItemNotReceivedEligible':
                $eligibilityStatus = Mage::getStoreConfig(
                    'buckaroo/buckaroo3extended_payconiq/sellers_protection_itemnotreceived_eligible',
                    $order->getStoreId()
                );
            ...
            case 'UnauthorizedPaymentEligible':
                $eligibilityStatus = Mage::getStoreConfig(
                    'buckaroo/buckaroo3extended_payconiq/sellers_protection_unauthorizedpayment_eligible',
                    $order->getStoreId()
                );
            ...
            case 'None':
                $eligibilityStatus = Mage::getStoreConfig(
                    'buckaroo/buckaroo3extended_payconiq/sellers_protection_ineligible',
                    $order->getStoreId()
                );
